...
# Versión activa: V37.72 · Última actualización: 2026-09-22 (fix checkpoint descarga incremental ventas + bug redirección .bat + panel Centro de Control)
...
- Versión activa: V37.72

### Historial de deploys (resumen — detalle completo en _HISTORICO\)
- V37.72 (2026-09-22): **Fix root cause — checkpoint de descarga incremental de ventas congelado 6 semanas + bug de redirección en 2 .bat.** (1) `VENTAS EL MANZANO\descargar_ventas_erp.py` → `_max_fecha_producto()` solo parseaba `F.Emision` con formato `%d/%m/%Y` (barras); las filas nuevas (desde que se agregó el path HTTP directo `_descargar_reporte_http`, más rápido que Playwright) vienen con `%d-%m-%Y` (guiones), y el parseo con barras fallaba silenciosamente (try/except: pass) para esas filas. Resultado: el máximo calculado quedó congelado en 2026-08-08 durante ~6 semanas aunque el archivo SÍ tenía datos frescos — cada corrida repetía una descarga cada vez más grande (llegó a 38 días laborales) que se deduplicaba a 0 filas nuevas reales. Fix: probar ambos formatos, igual que ya hacía la lógica gemela del xlsx de cliente. Verificado en vivo: antes del fix, descarga de 11.344 filas → 0 nuevas tras dedup; después del fix, `_max_fecha_producto()` detecta correctamente 2026-09-22 y la descarga incremental vuelve a pedir solo el día en curso. Se agregó también un guardrail de log (`[ALERTA]` en `_combinar_prod()`) para detectar en el futuro un escenario de 0 filas scrapeadas (sesión ERP expirada / reporte vacío), confirmado con Gemini antes de aplicar. **Nota:** `VENTAS EL MANZANO/` está gitignoreada a propósito desde la limpieza de seguridad 2026-06-02 (igual que `CATALOGO PRODUCTOS/` y `FLUJOS/`) — este fix vive en `E:\ferreteria-oviedo\` y su espejo `E:\git-sync\`, pero nunca llega a GitHub, por diseño. (2) Se instaló el paquete `holidays` (faltaba en el Python portable) — antes el cálculo de días laborales de la descarga incremental solo excluía domingos, ahora excluye también feriados chilenos reales (Fiestas Patrias, etc.), tal como opera el negocio. (3) `ACTUALIZAR_TODO.bat` (líneas 293 y 527) y `ACTUALIZAR_TODO_AUTO.bat` (línea 182): un `->` sin escapar dentro de un `echo` de caja (`(OC->recepcion)`, `Bot -> Firebase`) hacía que cmd.exe interpretara el `>` como redirección real, creando archivos basura sueltos en la raíz del proyecto en cada corrida (`recepcion)`, `Firebase`, con un fragmento de texto truncado adentro) — corregido escapando `^>`; archivos basura eliminados. (4) `E:\CONOCIMIENTO DEL NEGOCIO\control-center\server.js`: filtro nuevo que oculta en la consola web el aviso inofensivo "No es compatible la redirección de entradas" (de `timeout`/`choice`/`pause` corriendo con stdin ignorado) — solo cosmético, no toca ningún `.bat` ni su comportamiento manual. Ambos fixes de `.bat` y el del panel confirmados con Gemini antes de aplicar. (5) **Orden de raíz**: `README.md`, `IDS_REFERENCIA.md`, `MAPA_FLUJO_PROYECTOS.md`, `pipeline-datos-mapa.html`, `ORIENTACION_MARGEN_COSTO_20260909.md` → movidos a `docs\` (cero dependencias de ruta confirmadas antes de mover; `ACTUALIZAR_GITHUB.bat` actualizado con un robocopy adicional para los 3 que van a git-sync). Se archivaron fuera del proyecto (`E:\_ARCHIVO_FERRETERIA\`): `_HISTORICO\` completo (68 archivos, 0 referencias del pipeline activo) y 80 `ventas_erp_*.xlsx` viejos de `VENTAS EL MANZANO\` (solo se conserva el más reciente de cada tipo, que ya trae todo el histórico acumulado). Se eliminaron todos los `__pycache__` del árbol. **NO se movieron** (decisión deliberada, no pendiente): los HTML/PWA/config de Firebase Hosting (`firebase.json` tiene `"public": "."`, sirve la raíz del repo tal cual — moverlos rompe las URLs públicas y los rewrites), las 4 imágenes (`cotizador.html` tiene una ruta absoluta hardcodeada a `logo_oviedo.jpg`), y los 3 scripts de utilidad (`validar_jsons.py`, `generate_catalogo_bot.ps1`, `generar_catalogo_cotizador_rotacion.ps1` — 6+ puntos de llamada repartidos en 3 `.bat`, incluyendo una ruta absoluta en `ACTUALIZAR_TODO_AUTO.bat`, riesgo real de romper el catálogo del bot por muy poco beneficio de orden).
...
- V37.65 (2026-09-02): fix session kick panel-cliente por stale activo:false (ignorar docs con activo:false de más de 3 min); NFD search en panel-cliente (búsqueda sin tildes). Commit 919ccf2.
- V37.65 (2026-09-03): fix margen=0 panel vendedor — causa raíz: GitHub Actions workflow (.github/workflows/firebase-deploy.yml) desplegaba desde git-sync en cada push a main; Datos.json (3.6MB, .gitignore) no estaba en git → cada push lo borraba de Hosting → catálogo fallaba → caché localStorage compact sin campo CP → margenReal=null para todos. Fix: workflow cambiado a workflow_dispatch (solo manual). Deploy restauró Datos.json. ACTUALIZAR_TODO.bat pipeline sep-2026 completo: 6126 productos, ventas-manzano-2026-09.json (573KB), deploy 198 archivos, commit 7a4aa9c. restaurar_enviados_firestore.py confirmado: 115 enviados en Firestore (PEM:70 + SEM:45). GitHub push: 93e1f55.
- V37.71 (2026-09-21): **Fix infra — cuentas Firebase y GitHub cruzadas con proyecto Cabrasgo.** `ACTUALIZAR_TODO.bat` fallaba: `firebase deploy` con 403 (CLI logueado con `alejandrog45@gmail.com` de Cabrasgo en vez de `ferreteriaoviedo.elmanzano@gmail.com`, dueña real del proyecto Firebase `ferreteria-oviedo`) y luego el commit a GitHub con 403 similar (`git` autenticado como `alejandrog45-svg` sin permiso sobre `oviedoem/ferreteria-oviedo`). Fix sin tocar Cabrasgo: (1) `firebase login:add` sumó la cuenta correcta sin desloguear la otra; las 4 líneas `firebase deploy` de `ACTUALIZAR_TODO.bat` ahora llevan `--account ferreteriaoviedo.elmanzano@gmail.com`. (2) `E:\config\gitconfig` (global, compartido con LITUECHE/APP-INVENTARIO) — agregado bloque `[credential "https://github.com/oviedoem"]` con `useHttpPath = true`, así Git guarda una credencial aparte solo para rutas `oviedoem/*` sin afectar la entrada genérica de `github.com` que usa Cabrasgo; reautenticado vía OAuth con la cuenta correcta. Aparte, `descargar_erp.py` (PASO 1A) fallaba login SSRS por clave de `agonzalez` vencida en `CATALOGO PRODUCTOS\scripts\credenciales_erp.ini` — dueño actualizó la clave, login verificado OK. Pipeline corrido completo esa noche: ERP✅ SQL Server✅ Firebase✅ GitHub✅ (commit `f69f7fe`). Ver memoria `bug-firebase-deploy-cuenta-incorrecta-cabrasgo`. Sin cambios de panel/HTML/JS — solo scripts de infraestructura.
- V37.71 (2026-09-10): **Análisis de Bodegas — detalle multi-documento, filtro por persona (CEM), Excel con color/autofiltro real verificado.** Sesión enfocada en el menú "Análisis de Bodegas — IEM/RCE/CEM/ICD/GEM/TEM" (`panel-admin.html`), pedida por el dueño para auditar consistencia antes de confirmar nada. (1) **Subfilas con detalle completo**: cuando el stock de un código viene de varios documentos de ingreso (chevron ▶ de `documentosGRT`), antes solo se veían Tipo Doc/Folio/Cantidad/Fecha/Días/Obs. en blanco — ahora `vadmFiltrarBodFem` completa también Bodega/Código/Descripción/Costo unit./Valorizado por documento. Verificado con SQL en vivo (VPN activa) contra `M_DOCUMENTOS_DETALLE`: código 26297 (CEM) exacto — 1 unidad/118 días + 86 unidades/169 días, coincide con los documentos reales del ERP. (2) **Filtro "Filtrar por persona (CEM)"**: nuevo selector que extrae nombres de la observación libre del ERP (patrón `"POR FACTURAR <nombre>"`, detección dinámica vía regex, no lista fija) — filtra la tabla y muestra KPI de pendientes+$ acumulado por persona (ej. Marisel: 58 pendientes/$2.116.778, Lara: 30/$1.458.743). Solo aplica a CEM (única bodega con ese patrón en observación). Corregido en el desarrollo: el regex inicial capturaba "por" como nombre falso en `"FACTURADO POR CEM"` (facturado=ya hecho, no pendiente) — excluido con negative lookahead. (3) **Fix `_x000d_` en Excel**: `bfExportExcel` escribía observaciones con `\r\n` crudo del ERP directo a la celda (SheetJS) — Excel/openpyxl mostraban el texto literal `_x000d_` en vez de salto de línea; saneado con `.replace(/\r\n?/g,' ')`. (4) **Reescritura completa de `bfExportExcel`**: el dueño reportó que intentos anteriores de poner autofiltro en el Excel de este panel "se rompían al abrir" — se migró de SheetJS (`XLSX.utils.aoa_to_sheet`, sin soporte de color en la versión gratuita, confirmado con prueba real: relleno/negrita no se guardan) al patrón SpreadsheetML ya usado en `vadmDescargarExcel` (mismo proyecto): encabezado azul marino `#1A1D3A`, filas TOTAL en negrita, filas de documento en lavanda/cursiva `#EEEDFE`, pesos con separador de miles, AutoFilter + fila congelada. Incluye ahora los documentos de `documentosGRT` como filas separadas (antes solo se exportaba la fila principal). **Verificado con Excel real vía automatización COM** (no solo lectura de código): abrió sin aviso de reparación, `AutoFilterMode=True`, `FreezePanes=True`, colores de celda leídos coinciden exacto con los hex configurados (header `3808538`=`#1A1D3A` en BGR, filas documento `16707054`=`#EEEDFE` en BGR). (5) **Verificación de consistencia SQL en vivo, 6 bodegas** (2 agentes en paralelo + verificación propia en CEM): 51+ códigos cruzados contra `R_STOCK_PRODUCTOS`/`M_PRODUCTOS`/`M_DOCUMENTOS_DETALLE`, incluyendo casos de stock negativo en GEM/TEM (diseño ERP) y acumulación LIFO multi-documento — **0 discrepancias en las 6 bodegas**. Sin cambios de pipeline/SQL/JSON — todo el trabajo fue en `panel-admin.html` (capa de presentación).

- V37.70 (2026-09-10): **Menú "⚠️ Margen Negativo" en Panel Admin y Panel Vendedor + pipeline corrido completo en vivo.** Nuevo sub-menú en Panel Admin (grupo Análisis, `panel-admin.html`) que separa ventas reales (Boleta/Factura, Neto>0 & margen<0) vendidas bajo costo de las Notas de Crédito (devoluciones, excluidas del cálculo principal — se muestran aparte como contexto). Filtros: fecha desde/hasta, marca, hiperFamilia, familia, subFamilia, buscador libre (código/descripción/marca/cliente/vendedor/categoría). KPI "Margen CON estas ventas" vs "Margen SIN estas ventas" (global y por vendedor), tabla detalle (cliente/RUT/fecha/folio/documento/código/descripción/cantidad/costo/neto/margen), exportar Excel. Usa `margen`/`margenPct` directo, nunca recalculado (misma regla de siempre). **Equivalente agregado en Panel Vendedor** (`index.html`, dentro de "Mis Ventas"): bloque "🔻 Ventas bajo costo" con KPI antes/después y detalle de sus propias líneas bajo costo, mismo criterio de exclusión de NC. Motivado por pedido del dueño de auditar negativos reales tras confundir inicialmente NC con ventas bajo costo (corregido en la sesión). **Validado contra datos reales:** archivo SSRS real del ERP (`reportes_ventasporvendedor`, agosto 2026, vendedor rvidal) bajado por el dueño confirmó el mismo patrón — 63 líneas reales bajo costo vs 28 NC mezcladas en el export crudo; código 26191 (MALLA TIPO ACMA C92) repetido con -34.9% exacto en ambas fuentes. **Pipeline corrido completo en vivo** (`ACTUALIZAR_TODO.bat`, consola visible, monitoreado paso a paso): probado en vivo el fix del `errorlevel` de PASO 3 (pendiente de la sesión anterior) — respondió N correctamente. `validar_jsons.py` OK en todos los JSONs. Deploy Firebase + catálogo bot + commit GitHub exitosos (commit 1422bec). Verificado en producción vía Chrome real: menú funciona con datos reales, cifras cruzadas contra el ERP coinciden. Aviso no bloqueante detectado y diagnosticado (no corregido, no requiere código): `actualizar_config_precios.py` falla con HTTP 401 al refrescar el token de Firebase CLI — causa es una sesión de `firebase login` vencida en este equipo, no un bug; fix es correr `firebase login --reauth` una vez, fuera del pipeline.

- V37.69 (2026-09-10): **Revisión completa de scripts del pipeline (2 agentes en paralelo, 15 scripts, 5017 líneas) antes de correr `ACTUALIZAR_TODO.bat`.** Sin bugs bloqueantes — verificado que el mismo patrón de bug de `ENC_FILE` (ver fix de `descargar_costo_promedio.py` abajo) NO se repite en ningún otro script (los 15 restantes resuelven la ruta de credenciales correctamente, confirmado contando `.parent` y verificando que `E:\config\credenciales_db.enc` existe). Hallazgos menores corregidos: (1) `csv_a_json.py` — `MEM_BOD` faltaba en la lista de reconversión numérica (agregada, consistencia con `xlsx_a_csv.py`); (2) `csv_a_json.py` — no validaba que existieran columnas críticas (`codigo`, `descripcion`, `precioiva`) antes de generar `Datos.json` (6130 productos, consumido por TODO el sistema incluido el bot) — ahora falla fuerte con `SystemExit` en vez de generar un catálogo incompleto en silencio; verificado corriendo el script real contra el `Datos.csv` actual, sigue generando los 6130 productos sin cambios; (3) `descargar_bod.py`, `descargar_pedidos.py`, `descargar_despachos.py` — no llamaban `DATA_DIR.mkdir()` antes de escribir en la carpeta del token rotativo (a diferencia de sus 4 scripts hermanos que sí lo hacían) — agregado `mkdir(parents=True, exist_ok=True)` para que no fallen con `FileNotFoundError` si el token se rotó recién. Caso investigado y descartado como bug: `descargar_ventas_enrich.py` escribe `xlsm-enrich.json` en `data/` raíz (no en la carpeta del token) — verificado que es diseño intencional, coincide exacto con cómo lo lee `main.py` (`normalizar_totales_sql`) y con cómo lo respalda `ACTUALIZAR_TODO.bat` (`copy /Y data\xlsm-enrich.json...`).

- V37.69 (2026-09-10): **Fix de seguridad en `descargar_costo_promedio.py`** — `ENC_FILE` (línea 44) calculaba mal la ruta con `.parent.parent` en vez de `.parent.parent.parent` (patrón usado por todos sus scripts hermanos en BODEGAS\), resolviendo a `E:\ferreteria-oviedo\config\credenciales_db.enc` (no existe) en vez de `E:\config\credenciales_db.enc` (real, cifrado DPAPI). El script funcionaba igual porque `leer_credenciales()` cae en silencio al `.ini` en texto plano — sin loguear ni un WARN — violando la regla de CLAUDE.md de usar siempre credenciales cifradas. No bloqueaba el pipeline, pero exponía credenciales en texto plano de forma permanente y silenciosa en cada corrida. Corregido y verificado: `ENC_FILE` ahora resuelve a `E:\config\credenciales_db.enc` (confirmado que existe). Encontrado por un agente de auditoría dedicado a `main.py`+`descargar_costo_promedio.py` antes de correr `ACTUALIZAR_TODO.bat`. Pendiente de verificar tras la próxima corrida real: si la query SQL de costo promedio realmente matchea contra los códigos SSRS que usa `main.py` (formato de `CODIGO_TECNICO` no verificable sin conexión SQL desde este entorno) — revisar el log `[OK] N pares numero+codigo con costo promedio real`.

- V37.69 (2026-09-10): **Fix crítico en `validar_jsons.py` — el gate de deploy validaba archivos abandonados.** Revisión línea por línea de `ACTUALIZAR_TODO.bat`/`_AUTO.bat` a pedido del dueño (previo a correr el pipeline) encontró: (1) el schema de `validar_jsons.py` no incluía `ventas-manzano-2026-07/08/09.json` (meses recientes sin protección); (2) **hallazgo mayor** — TODAS las entradas `ventas-manzano-*` del schema (incluida `validar_ventas_vs_enrich()`) apuntaban a `data/` raíz, pero desde el fix V37.65 (2026-09-03) `main.py`/`descargar_ventas_erp.py` escriben en `data/<token>/`. Verificado con `Get-Item`: el archivo en raíz llevaba **7 días congelado** (03-09-2026, 41MB) mientras el real en `data/<token>/` seguía actualizándose (09-09-2026, 42MB) — el validador daba "OK" sobre datos abandonados, nunca sobre los reales que se despliegan. Fix: las 12 entradas `ventas-manzano-*` (meta, año, 9 meses) y `validar_ventas_vs_enrich()` ahora usan `BOD_DIR` (resuelve `.token-actual`, mismo mecanismo que ya usaban `bod-*.json`) en vez de `DATA_DIR`. **Verificado corriendo el script real** (no solo lectura de código): 65.417 registros en `ventas-manzano.json`, 2.511 en el de septiembre, 0/22.675 divergencias en el cruce contra SQL — todo `RESULTADO: OK` contra los datos vivos reales. Sin este fix, un pipeline corrido hoy habría desplegado con el gate de seguridad "ciego" a los datos que realmente se publican. `E:\git-sync\validar_jsons.py` es un espejo de solo lectura (robocopy vía `ACTUALIZAR_GITHUB.bat`) — se actualiza solo, no se tocó directamente.

- V37.69 (2026-09-10): **Auditoría + fix del bug de signo en 37 funciones + drilldown por producto en 21 menús de venta.** Continuación de V37.68: se auditaron los 32 menús de Panel Admin pendientes (agentes en paralelo) contra 2 reglas — usar `margen` directo (nunca `neto-costo`) y usar costo promedio real SQL (nunca `cat.get('costo')`). Hallazgo: 13 menús más tenían el mismo bug de signo ya corregido ayer en 5 funciones (`neto>0?(margen/neto)*100:0` ocultaba pérdidas cuando neto≤0) — corregidos uno por uno con TOCO/ARCHIVO/RAZÓN: `vadmRenderHora`, `vadmRenderMarcas`, `vadmRenderRankingMarca`, `vadmRenderVendRank`, `vadmRenderComparativa`, `vadmRenderClientes`, `vadmRenderFacturacion`, `_arKpis`, `vadmRenderArbolHeat`, `vadmRenderEstaciones`, `vadmBuscarEntreFechas`, `vadmRenderPagoAnalisis`, `vadmRenderPagoRankings`. Encontrados de paso y también corregidos (autorizado por el dueño): `vadmRenderAnalisis`, `vadmRenderTop`, `vadmRenderVendPro`, y 16 funciones `_vadmHtmlEmail*` (plantillas de los correos "Enviar por correo", 33 ocurrencias, aplicadas con script Node determinista dado el volumen). Total: 37 funciones corregidas, verificadas con datos reales de `ventas-manzano-2026-09.json` (caso real: cliente KLEVERONE, neto=-$39.706, margen=-$17.781 → antes mostraba 0.0%, ahora -44.8%). **Drilldown clickeable por producto** agregado/reforzado en 21 menús de venta (clientes, vendrank, comparativa, topMarcas×4-subtablas, rankingmarca, estaciones, facturacion, entrefechas, pagoanalisis, pagorankings, pagotemporal, arboltabla, arbolheat, hora, nc, marcavend2, impacto, preciodiff, marcavend, sector) — clic en una fila/celda abre modal con desglose hasta código+descripción, cantidad, neto, costo, utilidad y margen%; todos verificados matemáticamente exactos contra `ventas-manzano-2026-09.json` y `2026.json` antes de cerrar cada uno. `arbol` (descomposición Precio×Volumen) queda sin drilldown — no es tabla de productos. `tipodoc` ya tenía el drilldown completo desde V37.68. Bug adicional encontrado y corregido en `vadmRenderEstaciones`: 37 líneas ($247.132, sin campo `codigo`) quedaban fuera del agrupador por producto — ahora se incluyen bajo `(sin código)`. **Menús de stock (costo promedio, no drilldown de venta):** confirmado con `flujo-stock-justime.html` completo que estos menús son sobre Disp/Fís/Ped/Tránsito, no margen — `sobrestock/transito/bajrot/merma/stockconsulta/despachos` ya mostraban costo promedio real; **`informe-stock` corregido** — el costo real ya se descargaba (`_construirDatos()`) pero nunca se mostraba, se agregaron columnas "Costo Prom." y "Valor Fís." (Físico×costo) + fila de total. `quiebre`/`recepciones`/`mem` no aplican (no valorizan stock por diseño). **Sin cambios de pipeline, SQL, ni JSON de catálogo** — todo el trabajo fue en `panel-admin.html` (capa de presentación); el catálogo que alimenta al bot (`Datos.json`/`catalogo-bot.json`) no se tocó. **Sin deploy** — pendiente de `ACTUALIZAR_TODO.bat`, decisión del dueño. Limitación encontrada: no se pudo verificar visualmente post-login en `localhost:8799` — Firebase bloquea ese dominio (`auth/requests-from-referer-...-are-blocked`), verificación hecha con datos reales del pipeline en su lugar. Artefacto con el detalle completo: https://claude.ai/code/artifact/8bc1b008-0940-4725-9172-881af263b31d

- V37.68 (2026-09-09): **Costo promedio real (sin fallback ni cálculo).** Nuevo `BODEGAS/descargar_costo_promedio.py` (PASO 1L en ambos bats) trae costo promedio real por línea (numero+código) directo de `M_DOCUMENTOS_DETALLE` en SQL Server — `main.py` (`consolidar()`) elimina el fallback a `cat.get('costo')` (costo actual del catálogo); orden ahora: costo real SQL → costo de línea SSRS → nunca inventado. Verificado exacto contra ERP real (código 72744: $722,22 vs $722; SOLC0010: $31.532,62 vs $31.533) y contra el caso de margen negativo de la sesión (código 25989): el costo ya era correcto, confirmando que la caída de margen es real, no un artefacto de datos. **Margen por Tipo de Documento con drilldown completo:** nueva tabla clickeable (Boleta/Factura/NC) en Análisis→Categorías (Admin) y Mis Ventas (Vendedor) — clic filtra TODO el resto de la vista (cards, ranking, HiperFam→Familia→SubFam→Producto) a ese tipo de documento, reutilizando el drilldown existente sin duplicar lógica. Filtro de rango de margen unificado entre Admin y Vendedor (mismo criterio "Solo negativos" en ambos paneles, aplicado también a la tabla de Tipo de Documento). **Fix `calcSocio()` (Panel Vendedor, herramienta Clientes PRO):** cuando el costo promedio real es 0 (confirmado en JustWeb: 74 de 6130 productos con OC pendiente de recepción o sin OC, también con stock=0 en todas las bodegas), la función ya NO estima un costo falso desde el precio Socio (`SIva/1.19`) — muestra "⚠️ Sin costo real (OC pendiente)" en vez de un precio inventado. A margen=0 sigue mostrando el precio Retail real sin necesitar costo. **Vendedora nueva:** Vanessa Vásquez (`vanessa.vasquez` ERP → `vvsquz` Gmail) agregada a `VEN_CONFIG` (panel-admin.html + index.html) y `ERP_TO_GMAIL` (main.py). Commit: ver push de cierre de sesión.
- V37.67 (2026-09-09): Análisis de margen por categoría — panel-admin.html tab Análisis→Categorías (`vadmBuildCatTable`) agrega filtro de rango de margen % (botón "Solo negativos") para aislar HiperFamilia/Familia/SubFamilia que arrastran el margen a la baja; index.html (Panel Vendedor→Mis Ventas) agrega tabla "Margen por categoría (peor primero)" reutilizando el mismo fetch de ventas-manzano.json que ya llegaba al navegador (sin descarga nueva). Diagnóstico real ago-sep-2026: caída de margen (28%→25%→24%) explicada por MADERA y MATERIALES DE CONSTRUCCION (las 2 categorías de mayor volumen) perdiendo margen simultáneamente — causas puntuales: código 150124 (terciado nuevo, alto volumen/bajo margen) y código 25989 (Volcanita ST 15mm, margen cayó a -53.6%, posible error de costo/precio en ERP a revisar). Hallazgo técnico: el campo `costo` de ventas-manzano*.json no es confiable para meses históricos (main.py:509-510 usa costo ACTUAL del catálogo como fallback cuando la línea de venta no trae costo propio) — usar siempre el campo `margen`/`margenPct` directo, nunca recalcular como valorNeto-costo. Sin cambios de pipeline ni SQL nuevo. Sin deploy (pendiente próximo ACTUALIZAR_TODO.bat).
- V37.66 (2026-09-03): fix ventas de septiembre ausentes en Análisis/Ventas por Vendedor — causa raíz: main.py `guardar_json()` y descargar_ventas_erp.py `_generar_jsons()` escribían ventas-manzano*.json en `data/` raíz, pero panel-admin lee via `dataUrl()` desde `data/<token>/` (post-eliminación de rotar_token en V37.64 el token sigue rotando pero los scripts de ventas no seguían el patrón token de descargar_bod.py). Fix: ambos scripts ahora resuelven `.token-actual` y escriben en `data/<token>/` (mismo patrón que descargar_bod.py). Pipeline completo corrido con VPN activa: Precio+Stock/Ventas/Deploy Firebase/Catálogo Bot/Commit GitHub todos OK. Verificado en Hosting: ventas-manzano-meta.json (63752 registros, año 2026) y ventas-manzano-2026-09.json (1000 registros sep) presentes en data/<token>/. pipeline-datos-mapa.html documentado. Commit 4bb2f79.
...
**REGLA (confirmada por el dueño 2026-08-31): el deploy NUNCA se ejecuta manualmente
ni por el agente. El deploy SOLO se hace dentro del pipeline `ACTUALIZAR_TODO.bat`
(PASO 4). Si hay cambios pendientes de publicar, se deja constancia y se corre el
pipeline — nunca `firebase deploy` suelto.**
...
2. Si algún archivo es más nuevo → reportar "deploy pendiente" (lo publica el próximo ACTUALIZAR_TODO.bat)
...
**Estado actual — carpeta token fija (2026-09-01):**
- Los JSON sensibles viven en `data/<token-fijo>/` — la rotación automática fue eliminada
- El token activo está en `data/.token-actual` y en Firestore `dataAccessToken/current`
- `panel-admin.html` e `index.html` leen el token tras login con `dataUrl(nombre)`
- La rotación de token (`rotar_token_data.py`) fue eliminada del pipeline — causaba 404 diarios
- Revisión de seguridad de datos pendiente para sesión futura
