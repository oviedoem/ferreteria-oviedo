...
### Fix 2026-07-01 (sesión 3p) — bug real: días/fecha de registro incorrectos en TEM (y latente en GEM)
- **Reportado por el dueño con evidencia cruzada**: comparó `panel-admin` (TRUP0156 @ TEM mostraba fecha 07/10/2022, 1363 días) contra el reporte SSRS "tarjeta de inventario (tem) transito el manzano" del propio ERP, que mostraba la entrada real más reciente como "Compra Guía Recepción 691790, 26-06-2026". Señaló también que CEM/RCE/IEM "tienen la fórmula exacta" (no fallan) y pidió revisar GEM por el mismo motivo, notando que ahí los movimientos van con signo +/- y "el resultado final se netea".
- **Causa raíz confirmada con SQL en vivo** (`BODEGAS\descargar_bod.py`, CTE `ENTRADAS`): el JOIN exigía `N.IDSUCURSAL = A.IDSUCURSAL` (documento debe compartir la sucursal "hogar" de la bodega, ej. '04' El Manzano). Pero el documento real GRC 691790 (26-06-2026, recepción de 6 unidades) quedó grabado con `IDSUCURSAL='02'` (Isabel Riquelme) — normal en el ERP para traslados entre sucursales hacia bodegas logísticas compartidas como TEM/GEM. El filtro descartaba sistemáticamente toda la actividad reciente real y caía hasta el primer documento antiguo que calzara por casualidad con sucursal='04' (un GRT de 2022). Confirmado con query sin el filtro: la traza completa muestra un patrón recurrente GRC(entra, sucursal 02)/GTS(sale) cada pocos meses — exactamente el patrón "positivo/negativo que se netea" que describió el dueño para GEM, mismo bug, bodega distinta. CEM/RCE/IEM no lo sufrían simplemente porque sus documentos de entrada históricamente sí quedan con sucursal='04' — no había una fórmula distinta, es el mismo código.
- **Fix**: quitada la condición `AND N.IDSUCURSAL = A.IDSUCURSAL` del JOIN de `ENTRADAS` (`IDBODEGA` ya identifica la bodega física de forma única en `P_BODEGAS`, no hace falta reforzar con sucursal). Efecto colateral encontrado y corregido de paso: el `LEFT JOIN` a `M_Documentos_Encabezado_Observacion` también usaba `G.IDSUCURSAL = A.IDSUCURSAL` — con el filtro de arriba quitado, documentos con sucursal distinta (como el GRC de ejemplo) habrían perdido su observación real; cambiado a `G.IDSUCURSAL = N.IDSUCURSAL` (la sucursal del documento, no la de la bodega).
- **Verificado en vivo**: recorrido `descargar_bod.py` completo. TRUP0156 @ TEM ahora sale `tipoDoc=GRC, folio=691790, fecha=26/06/2026, días=5`, con la observación real de la recepción incluida (antes vacía). IEM/RCE/CEM/ICD mantienen el mismo conteo de registros (100/53/6/3) — el fix no las altera, confirma que ya venían bien. TEM pasó de 5→70 registros (mucha actividad recuperada); GEM se mantuvo en 155 registros pero con rango de antigüedad sano (9–168 días, sin negativos en disp/fisico — 0 encontrados).
- **Nota importante de arquitectura descubierta en el camino** (no es bug, pero hay que recordarlo): `R_STOCK_PRODUCTOS` (la tabla de saldos que usa este pipeline) **se actualiza 1 vez al día (batch nocturno, ~22:00)**, según indicó el dueño — no es tiempo real. Un traslado hecho hoy (ej. TRUP0156 salió de TEM hacia IEM el mismo 01-07-2026) no se refleja en `ST_FISICO`/`ST_DISPONIBLE` hasta la corrida de esa noche. Esto es inherente al ERP, no algo que este pipeline pueda corregir — explica por qué una consulta SQL en vivo puede diferir de la pantalla "Consulta Stock Rápido" del ERP en una ventana de pocas horas.
- Redeploy manual de los 6 JSON corregidos dentro de la carpeta `data/<token>/` activa (mismo patrón de sesión 3o — sin rotar token, sin correr `rotar_token_data.py` aislado). 2 redeploys de hosting, verificados con `curl` contra producción.
- `BODEGAS\descargar_bod.py` no se sube a git (`BODEGAS/` está en `.gitignore`) — este fix vive solo local, documentado aquí y en memoria para que no se pierda entre PCs.

### Fix 2026-07-01 (sesión 3q) — Análisis de Bodegas: stock negativo (GEM/CEM) oculto por completo
- **Reportado por el dueño con listado ERP real** (bodega GEM, ej. códigos 72009 `BOTA GOMA BLACK BULL N°40` disp=-1, `MALC0020` disp=-9): pidió que los valores negativos de Disp/Físico se muestren (no se oculten) y que el $ Valorizado salga también en negativo, destacado en rojo.
- **Causa raíz doble en `BODEGAS\descargar_bod.py`**:
  1. SQL: `WHERE ... AND ISNULL(A.ST_FISICO, 0) >= 1` excluía por completo cualquier fila con físico 0 o negativo — el producto ni siquiera llegaba a Python.
  2. Python (`_deduplicar_y_acumular`): `if total_fisico <= 0: continue` descartaba el producto igual aunque la SQL lo hubiera traído; y el loop de acumulación (`while acum < total_fisico`) se rompe con un total negativo — `acum=0 >= total_fisico` (negativo) es verdadero de entrada, así que nunca se agregaba ni un documento.
- **Fix**:
  - SQL: `>= 1` → `<> 0` (incluye negativo y positivo, sigue excluyendo cero).
  - Python: `total_fisico == 0: continue` (ya no descarta negativos); rama nueva `if total_fisico > 0: acumular... else: tomar solo el documento más reciente` (no tiene sentido "acumular cantidades positivas hasta cubrir un total negativo" — el signo del stock ya viene de `A.ST_FISICO`/`A.ST_DISPONIBLE` de la fila completa, no de sumar `cantidad`).
  - `panel-admin.html` (`vadmFiltrarBodFem()`): celdas Disp/Fís en rojo (`color:#dc2626;font-weight:700`) cuando son negativas; celda Valorizado ya no las oculta detrás de `val>0?...:'—'` (mostraba "—" para cualquier valor negativo, tapando justo lo que el dueño pedía ver) — ahora `val!==0?...:'—'`, en rojo si `val<0`.
- **Verificado en vivo**: `descargar_bod.py` recorrido completo. GEM pasó de 155→296 registros (incluye ~141 con disp/físico negativo), CEM 100→101, TEM 70→72; IEM/RCE/ICD sin cambio (no tienen stock negativo). Confirmados 2 códigos reales del listado del dueño: `72009` (-1, fecha 22/05/2026, obs "INGRESO DE MATERIAL SOBRANTE") y `MALC0020` (-9, fecha 04/06/2026, obs "-GEM A PEM / AJUSTE POST INVENTARIO GENERAL..."). Preview local (con token inyectado manualmente para bypasear el login) confirma render real: Disp/Fís en rojo, Valorizado `$-150.593` en rojo (antes hubiera mostrado "—"). Un código del listado (`26297`) no aparece — `R_STOCK_PRODUCTOS` no tiene fila viva para esa combinación código+bodega+sucursal ahora mismo (no es un bug de este fix, es ausencia de dato base).
- Redeploy manual de los 6 JSON dentro de `data/<token>/` activa, mismo patrón sin rotar token. Verificado con `curl` que la raíz pública sigue en 404.

### Fix 2026-07-01 (sesión 3r) — Informe Stock: Disp desactualizado (Excel manual) generaba Dif fantasma
- **Reportado por el dueño**: en Informe Stock, código SIKA0310/SEM mostraba Disp=0, Fís=14, Dif=14 (clickeable); el modal de drill-down decía "No hay despachos pendientes... si Dif≠0 viene de documentos anteriores, ejecute ACTUALIZAR_TODO.bat". Pidió revisar consistencia, y antes de decidir el fix pidió revisar 2 Excels de referencia: `_HISTORICO\ID DOC OVIEDO EM.xlsx` (glosario de tipos de documento) y `BODEGAS\Copia de Movimiento Stock.xlsx` (hoja FLUJO ERP — matriz oficial de movimientos Disponible/Contable/Fisico/Pedido por tipo de documento).
- **Confirmado con los Excels + SQL en vivo**: la matriz FLUJO ERP establece que Disponible SOLO baja cuando hay una Nota de Venta real, y ese movimiento SIEMPRE suma la columna Pedido en el mismo paso. En SQL, `R_STOCK_PRODUCTOS` mostraba Pedido=0 y ningún documento NVM/VMN/BVE/FVE pendiente para SIKA0310/SEM — es decir, según la propia fórmula del ERP, Disponible NO debería estar en 0. El modal de despachos respondía correctamente ("no hay documentos") a una pregunta que no debía hacerse: el Dif=14 no correspondía a ningún evento real.
- **Causa raíz**: "Disp" en Informe Stock viene de `CATALOGO PRODUCTOS/Datos.json`, generado por `procesar-actualizacion.py` desde `actualizar.xlsx` — un Excel que se **exporta manualmente** del ERP (no lo genera ningún script del pipeline). Mientras tanto, "Fís" en el mismo tab ya se sobreescribe con datos frescos de `informe-stock.json` (`generar_informe_stock.py`, que lee `raw_bloque1/2_YYYYMMDD.csv` generados por `descargar_erp.py`/SSRS el mismo día). Confirmado: el CSV de HOY (`raw_bloque1_20260701.csv`) ya traía `St_Disp=14` correcto para SIKA0310/SEM (igual a `R_STOCK_PRODUCTOS` en vivo) — el dato fresco **ya existía**, pero `generar_informe_stock.py` solo leía `St_Bod`/`St_Ped` de esa columna, nunca `St_Disp`, y el merge en `panel-admin.html` (`isbGenerar()` → `tryRender()`) solo sobreescribía `pem_bod/sem_bod/cem_bod/mem_bod` desde `informe-stock.json`, dejando `pem/sem/cem/mem` (Disponible) con el valor viejo del Excel manual.
- **Escala**: 25 de 4.765 productos revisados (~0.5%) con el mismo patrón (informe-stock.json con stock real, Datos.json en 0 para la misma bodega), concentrados en SEM.
- **Decisión del dueño**: priorizar el dato SQL/SSRS sobre el Excel manual (opción recomendada, auto-corrige sin depender de que alguien reexporte el Excel a tiempo).
- **Fix**:
  - `BODEGAS\generar_informe_stock.py` (V1.1): ahora también lee `St_Disp` del CSV y agrega `pem_disp/sem_disp/cem_disp/mem_disp` a `informe-stock.json`.
  - `panel-admin.html` (`isbGenerar()` → `tryRender()`): el merge que ya sobreescribía `_bod` (Físico) ahora también sobreescribe `window._vadmStockMap[cod].pem/sem/cem/mem` (Disponible) con `d.pem_disp/sem_disp/cem_disp/mem_disp` — mismo patrón, mismo punto de merge, solo se agregaron 4 líneas análogas a las 4 que ya existían para `_bod`.
- **Verificado en vivo**: `generar_informe_stock.py` recorrido completo (9.717 productos). SIKA0310 ahora sale `sem_disp:14, sem_bod:14` (antes Datos.json tenía `sem:0`). Confirmados los 9 códigos de la muestra anterior (incluyendo los 8 `HENK*`) con Disp=Fís correctos. Preview local (`window.isbGenerar()` con token inyectado) confirma render real: fila SIKA0310 ahora muestra Disp=14/Fís=14/Dif=0 (verde, antes ámbar con Dif=14).
- Nota: `_vadmStockMap` es una variable global de las protegidas (FO-005) — no se renombró ni eliminó, solo se le agregaron asignaciones a propiedades existentes (mismo patrón ya usado para `_bod`).
- Redeploy de `informe-stock.json` dentro de `data/<token>/` activa. V37.51.

### Fix 2026-07-01 (sesión 3s, cierre) — Encabezados de tabla sticky en todos los menús
- **Pedido del dueño**: extender el fix de encabezado sticky de Informe Stock (sesión 3o/3i previa, no documentada por error como parte de esta cadena) a "todos los menús" del panel.
- **Alcance**: se cubrieron los 18 `.tab-pane` (todos los ítems del menú lateral) y, dentro de `tab-ventas`, los 34 `.vadm-section` anidados (Quiebre, Sobrestock, Tránsito, Baja Rotación, Merma, Liquidación, Rankings, Análisis, Temporal, etc.) — todo lo que cuenta como "menú" en el sidebar.
- **Enfoque**: en vez de editar cada tabla generada dinámicamente por separado, se agregaron 2 reglas CSS scoped por clase compartida:
  - `.tab-pane .tbl th, .tab-pane .vtbl th, .tab-pane .vadm-rank-table th{position:sticky;top:60px;z-index:5}` — cubre la enorme mayoría de tablas dinámicas del panel (Quiebre/Sobrestock/Baja Rotación/Merma/Rankings/etc. usan estas 3 clases).
  - Excepción para tablas que viven dentro de modales con scroll propio (offset `top:0`, no `top:60px`): `#vadmSEModalBody`, `#cotizModalContent`, `#vadmVPModalBody`, `#vadmDocModalBody` — identificados revisando cada `getElementById('...Modal...')` del archivo y confirmando cuáles tienen `overflow-y:auto`/`overflow:auto` propio (serían las únicas 4 con `.tbl`/`.vtbl`/`.vadm-rank-table` renderizado adentro).
- **Tablas estáticas por id** (no cubiertas por las clases compartidas): `#dpTabla` (Despachos Pendientes), `#rpTabla` (Por Recepcionar), `#bfTabla` (Análisis de Bodegas), `#csTablaResult` (Consulta de Stock — resultados), `#csBodTabla` (Consulta de Stock — ficha por bodega, clase `cs-xls`), `#tcdThead` (Traspasos CD, ya tenía id propio en el `<thead>`) → `position:sticky;top:60px;z-index:15` agregado directo en el `<thead>` de cada una. `#reqStockTable` (Solicitud Stock) ya usaba la clase `vadm-rank-table`, quedó cubierta por la regla CSS sin cambios adicionales.
- **`top:60px`** = altura exacta de `.topbar` (`position:sticky;top:0;height:60px`, mismo offset que ya usa `.sidebar` en este archivo) — mismo patrón ya aplicado a Informe Stock.
- **No se tocó**: `vendConfigTable` (tabla de referencia corta en Config ERP, no es un listado que se desplace) y modales sin tablas de estas 3 clases (`#isbDetalleModal`, `#vadmEmailModal`, `#quotaModal`, etc. — construyen tablas con estilos inline propios, no usan `.tbl`/`.vtbl`/`.vadm-rank-table`, fuera de alcance).
- **Verificado**: computed style confirma `position:sticky;top:60px` en las 7 tablas estáticas. Prueba dinámica (crear un `<table class="vtbl">` de prueba dentro de un `.tab-pane` real vs. dentro de `#vadmSEModalBody`) confirma que la regla general aplica `top:60px` en el menú y la excepción aplica `top:0` dentro del modal — comportamiento exacto esperado, sin errores de consola.
- V37.52.

### Fix 2026-07-01 (sesión 3t, cierre real) — el fix de sticky headers de la sesión 3s (y el de Informe Stock de 3i) NO funcionaba de verdad
- **Reportado por el dueño con screenshot real**: en Merma MEM, el encabezado aparecía **debajo de la primera fila de datos** en vez de arriba ("todos los menu quedaron corridos en los titulos").
- **La verificación de la sesión 3s estaba incompleta**: solo se había confirmado `getComputedStyle` (que el CSS `position:sticky;top:60px` se aplicaba), pero eso **no prueba que el sticky funcione visualmente** — un elemento puede tener `position:sticky` computado y aun asi no "pegarse" nunca si su contenedor de referencia (containing block) no es el que uno cree. Nunca se hizo scroll real + screenshot hasta este fix.
- **Reproducido con scroll real** (forzando `#app` visible + ocultando `#loginScreen`, ya que estaba detrás del login): confirmado que el header se desprendía visualmente de su fila cuando el sticky se aplicaba a cada `<th>` individual en vez de al `<thead>` completo (bug real, corregido primero) — pero después de corregir eso, el header **seguía sin pegarse en absoluto** (se iba de pantalla con el resto del contenido).
- **Causa raíz real** (la de fondo, la que explica todo): los wrappers `.ven-adm-table-wrap`/`<div style="overflow-x:auto">` que envuelven TODAS estas tablas quedan clasificados por el navegador como "contenedor con scroll" — no por tener `overflow-y` explícito, sino por una regla del spec CSS: *si `overflow-x` no es `visible` y `overflow-y` sí lo es, el navegador computa `overflow-y` como `auto` también* (confirmado con `getComputedStyle`: `.ven-adm-table-wrap` mostraba `overflow-y:auto` aunque el CSS fuente solo declara `overflow-x:auto`). Eso convierte a ese wrapper — no la ventana — en el "containing block" de referencia para `position:sticky` de sus hijos. Como ese wrapper nunca tiene una altura acotada, su propio contenido nunca lo desborda, nunca hace scroll por sí mismo, y el `thead` sticky (que solo reacciona al scroll DE SU contenedor de referencia, no al de la ventana) nunca se activa — queda estático y se va de pantalla junto con todo lo demás al hacer scroll de la página. Verificado también que `#isbTabla` (Informe Stock, el primer "fix" de la sesión 3i) tenía exactamente el mismo problema — nunca se había probado con scroll real, solo con `getComputedStyle`.
- **Fix real**: en vez de pelear contra esa regla del navegador, se acepta y se usa: los wrappers ahora tienen `max-height:65vh;overflow-y:auto` explícito (para que sean ellos los que hacen scroll de verdad, con su propia barra), y el `thead` sticky pasa de `top:60px` a `top:0` (relativo al wrapper, no a la página). Selector usado para cubrir TODOS los wrappers sin enumerar cada uno manualmente: `div:has(> table.tbl), div:has(> table.vtbl), div:has(> table.vadm-rank-table)` + selectores puntuales para los 7 wrappers de tablas estáticas por id (`isbTabla`, `dpTabla`, `rpTabla`, `bfTabla`, `csTablaResult`, `csBodTabla`, y el wrapper de la tabla que contiene `#tcdThead`). `:has()` es CSS moderno con soporte universal en 2026 (Chrome 105+/Firefox 121+), no hace falta enumerar wrappers uno por uno.
- **Ya no hacen falta las excepciones de modales** (`#vadmSEModalBody` etc. de la sesión 3s) — con `top:0` universal y el wrapper como contenedor de scroll real en TODOS lados (menú o modal), el comportamiento es consistente sin necesitar reglas separadas para modales.
