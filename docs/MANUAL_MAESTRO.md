...
## ⚡ VERSIÓN ACTUAL: V33.6 (2026-05-18)

### Script principal recomendado
```
D:\ferreteria-oviedo\ACTUALIZAR_TODO.bat
```
Hace TODO en un solo paso:
1. Descarga precio + stock desde ERP (Playwright + HTTP)
2. Procesa catálogo → `Datos.json` (5.939 productos, incluye MEM)
3. Descarga ventas ERP → `ventas-manzano-YYYY.json` + `ventas-manzano-YYYY-MM.json` (split por año y mes)
4. Lee XLSM del Servidor 2 → `ventas-xlsm-sector.json` + `ranking-unidades.json` + `precios-diff.json`
5. Deploy Firebase Hosting

### Tarea automática diaria
La tarea programada de Windows ejecuta `ACTUALIZAR_TODO.bat` a las **19:00**.
Log en: `D:\ferreteria-oviedo\VENTAS EL MANZANO LOCAL\salida\auto_log.txt`

### Scripts anteriores (mantener por compatibilidad, NO usar como principal)
| Script | Estado |
|--------|--------|
| `ACTUALIZAR_Y_PUBLICAR.bat` | Legacy — sin XLSM ni split por mes |
| `VENTAS EL MANZANO LOCAL\PREPARAR_Y_PUBLICAR.bat` | Legacy — reemplazado |
| `VENTAS EL MANZANO LOCAL\ACTUALIZAR_AUTO.bat` | Legacy — reemplazado por tarea nueva |
