...
- **Versión activa:** V37.68 · 2026-09-09 · ver AGENTS.md (historial de deploys)
- **Última sesión (09-09-2026) — MUY EXTENSA, leer completa antes de tocar margen/costo/venta:** costo promedio real desde SQL (`descargar_costo_promedio.py` nuevo, PASO 1L) reemplaza el fallback a costo del catálogo actual — nunca más se calcula/estima costo en el pipeline. Margen por Tipo de Documento (Boleta/Factura/NC) con drilldown completo hasta código+descripción, clickeable, en Panel Admin Y Panel Vendedor, filtros unificados entre ambos. Fix `calcSocio()` en Panel Vendedor (Clientes PRO): ya no inventa costo desde precio Socio cuando el ERP no tiene costo real (confirmado en JustWeb: productos con OC pendiente/sin OC). Investigación completa de "líneas SSRS duplicadas" — CERRADA sin tocar código: es 1 documento aislado en ~300, no un patrón (ver artefacto). Vendedora nueva Vanessa Vásquez agregada. **Antes de la próxima sesión que toque paneles de venta: leer `memory/estado-sesion-20260909.md` completo** — tiene el detalle de cada fix, cómo se verificó, y qué queda pendiente (revisar todos los DEMÁS menús de venta que NO se tocaron hoy — ver prompt de continuación en esa misma memoria).
