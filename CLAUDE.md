...
- **Versión activa:** V37.72 · 2026-09-22 · ver AGENTS.md (historial de deploys)
- **Última sesión (22-09-2026):** Fix root-cause del checkpoint de descarga incremental de ventas, congelado ~6 semanas en 2026-08-08 — `descargar_ventas_erp.py` (`_max_fecha_producto()`) solo parseaba `F.Emision` en formato `dd/mm/yyyy`; las filas nuevas del path HTTP directo vienen en `dd-mm-yyyy` y se ignoraban silenciosamente. Corregido para probar ambos formatos, verificado en vivo (antes: 11.344 filas descargadas → 0 nuevas tras dedup; después: checkpoint detecta hoy correctamente y solo re-pide el día en curso). Se agregó guardrail de log `[ALERTA]` para detectar a futuro un scraping realmente vacío, y se instaló el paquete `holidays` (faltaba) para excluir feriados chilenos reales del cálculo de días laborales, no solo domingos. Además: 2 `.bat` (`ACTUALIZAR_TODO.bat`, `ACTUALIZAR_TODO_AUTO.bat`) tenían un `->` sin escapar en un `echo` que hacía que cmd.exe creara archivos basura en la raíz cada corrida (`recepcion)`, `Firebase`) — corregido y basura eliminada. Panel "Centro de Control Oviedo" (`E:\CONOCIMIENTO DEL NEGOCIO\control-center`): filtrado el aviso cosmético "No es compatible la redirección de entradas". Todos los fixes de lógica/comportamiento confirmados con Gemini antes de aplicar. **Nota:** `VENTAS EL MANZANO/` sigue gitignoreada a propósito desde 2026-06-02 (igual que `CATALOGO PRODUCTOS/`) — estos fixes viven en `E:\ferreteria-oviedo\` y su espejo `E:\git-sync\`, pero no llegan a GitHub, por diseño de seguridad ya documentado. **Ver `memory/estado-sesion-20260922.md` completo.**
- **Sesión previa (21-09-2026):** Fix infra — `ACTUALIZAR_TODO.bat` fallaba con 403 en Firebase deploy y en commit GitHub por cuenta cruzada con el proyecto Cabrasgo (CLI logueado con `alejandrog45`, no con `ferreteriaoviedo.elmanzano` dueña real de `ferreteria-oviedo`). Fix sin tocar Cabrasgo: `firebase login:add` + `--account` en las 4 llamadas deploy del bat; `E:\config\gitconfig` con bloque `credential "https://github.com/oviedoem"` (`useHttpPath=true`) para que Git guarde credencial aparte solo para esa organización. También clave ERP de `agonzalez` vencida (login SSRS fallaba) — dueño la actualizó, verificado OK. Pipeline completo corrido: ERP✅ SQL✅ Firebase✅ GitHub✅. Sin cambios de panel. Ver `memory/estado-sesion-20260921.md`.
- **Sesión previa (10-09-2026, continuación noche):** Análisis de Bodegas — subfilas con detalle completo por documento (Bodega/Código/Descripción/Costo/Valorizado, antes en blanco), filtro "Filtrar por persona (CEM)" con KPI de acumulado (extracción dinámica de nombres desde observación), fix `_x000d_` en Excel, y reescritura completa de `bfExportExcel` a SpreadsheetML con color/negrita/AutoFilter (el patrón SheetJS anterior no soporta color en su versión gratuita) — verificado abriendo el archivo real con Excel vía COM (sin aviso de reparación, autofiltro y colores confirmados). Consistencia SQL en vivo verificada en las 6 bodegas (51+ códigos, 0 discrepancias, 2 agentes en paralelo + verificación propia). **Ver `memory/estado-sesion-20260910.md` completo.**
