...
- **Versión activa:** V37.69 · 2026-09-10 · ver AGENTS.md (historial de deploys) · **DEPLOY HECHO, commit 60e6546**
- **Última sesión (10-09-2026):** auditoría de los 32 menús de venta pendientes + fix del bug de signo en margen negativo en 37 funciones (13 menús + 3 sueltas + 16 plantillas de email) + drilldown por producto agregado/reforzado en 21 menús de venta (todos verificados con datos reales, no solo lectura de código) + columna costo promedio en Informe Stock. Revisión línea por línea de `ACTUALIZAR_TODO.bat`/`_AUTO.bat` y los 17 scripts Python del pipeline antes de correr: encontrado y corregido bug real en `validar_jsons.py` (validaba `ventas-manzano*.json` abandonados en `data/` raíz, 7 días sin proteger los datos reales que se despliegan), credenciales en texto plano en `descargar_costo_promedio.py` (ruta mal calculada), y bug de `errorlevel` en cascada en `ACTUALIZAR_TODO.bat` (selección S/N/E de precios siempre terminaba en S). Pipeline corrido completo en vivo, monitoreado en tiempo real, deploy exitoso. **Ver `memory/estado-sesion-20260910.md` completo** para el detalle de cada fix y lo pendiente para la próxima sesión.
