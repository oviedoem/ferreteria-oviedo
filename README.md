...
```
ERP JustWeb (HTTP/SSRS)  +  SQL Server Foviedo  +  Intranet Blazor
        │
        ▼
...
descargar_despachos.py      → despachos-*.json                                            [SQL]
generar_informe_stock.py    → informe-stock.json  (físico + comprometido)                 [SSRS]
descargar_blazor_bodegas.py → recepciones-pendientes.json + despachos-pendientes-erp.json [Playwright]
...
> **Origen de los datos:** ERP/SSRS (real-time) aporta precios, stock y ventas; SQL Server (sincroniza 1×/día a las 22:00) aporta rut/razón social/sector, movimientos de bodega, pedidos y despachos; Playwright lee recepciones/despachos pendientes de la intranet en tiempo real.
>
